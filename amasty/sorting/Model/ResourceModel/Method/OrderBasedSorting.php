...
class OrderBasedSorting extends AbstractIndexMethod
{
    ...
    public function doReindex(): void
    {
        ...
        if ($resultInfo) {
            $rows = [];
            $count = 0;
            foreach ($resultInfo as $sortingRow) {
                $sortingColumnName = $this->getSortingColumnName();
                $rows[] = [
                    self::PRODUCT_ID => (int) $sortingRow[self::PRODUCT_ID],
                    self::STORE_ID => $sortingRow[self::STORE_ID],
                    $sortingColumnName => $sortingRow[$sortingColumnName]
                ];
                $count++;
