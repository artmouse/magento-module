...
class LazyConfig extends DataObject
{
    ...
    public const ENABLE_WEBP = [
        self::GENERAL => 'amoptimizer/replace_images_general/webp_resolutions',
        self::HOME => 'amoptimizer/replace_images_home/webp_resolutions',
        self::CATEGORIES => 'amoptimizer/replace_images_categories/webp_resolutions',
        self::PRODUCTS => 'amoptimizer/replace_images_products/webp_resolutions',
        self::CMS => 'amoptimizer/replace_images_cms/webp_resolutions'
    ];
    ...
    private function initSkipImages(): void
    {
        ...
        if ($this->isCustomLazyEnabled) {
            if ($this->configProvider->getConfig($this->configPath . ConfigProvider::PART_PRELOAD)) {
                $value = $this->configProvider->getConfig($this->configPath . ConfigProvider::PART_SKIP . $type);
            }
        } else {
    ...
    private function initLazyScript(): void
    {
        ...
        $this->setData(self::LAZY_SCRIPT, $value);
    }
    ...
    private function initImgOptimizerSupportThirdPartyImgAttributes($pageType)
    {
        ...
        if ($this->scopeConfig->getValue(self::ENABLE_WEBP[$scope], ScopeInterface::SCOPE_STORE)) {
