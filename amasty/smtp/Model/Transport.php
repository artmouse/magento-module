        ...
        try {
            ...
            if ($this->isTest || $isEnabledDelivery) {
                if ($this->_message instanceof \Zend_Mail) {
                    ...
                    if ($this->_message->getBody() || $this->_message->getBodyHtml()) {
                        parent::send($this->_message);
                    } else {
                        parent::send($this->_mailMessage);
                    }
                } else {
                    /** @var \Laminas\Mail\Transport\Smtp $laminasTransport */
                    $laminasTransport = $this->objectManager->create(\Laminas\Mail\Transport\Smtp::class);
                    $laminasSmtpOptions = new \Laminas\Mail\Transport\SmtpOptions(
                        [
                            'name' => $this->_name,
                            'host' => $this->_host,
                            'port' => $this->_port,
