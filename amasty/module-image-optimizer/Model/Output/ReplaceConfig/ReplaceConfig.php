...
class ReplaceConfig extends DataObject
{
    ...
    private function initIsReplaceImages(): void
    {
        $value = ($this->isEnabledCustomReplace)
            ? (bool)$this->configProvider->getConfig(
                $this->configPath . ConfigProvider::PART_REPLACE_WITH_WEBP
            )
            : $this->configProvider->isReplaceWithWebp();

        $this->setData(self::IS_REPLACE_IMAGES, $value);
    }
